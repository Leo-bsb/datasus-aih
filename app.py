...
@st.cache_resource
def build_db():
    """Executado uma vez por sessão. Cria o banco se não existir ou estiver vazio."""
    if DB_PATH.exists():
        # Verificar se o banco tem dados — pode ter sido criado vazio por erro anterior
        try:
            conn_test = sqlite3.connect(DB_PATH)
            n = conn_test.execute(
                "SELECT COUNT(*) FROM producao_hospitalar"
            ).fetchone()[0]
            conn_test.close()
            if n > 0:
                return True
            # Banco existe mas está vazio — recriar
            DB_PATH.unlink()
        except Exception:
            # Tabela não existe ainda — recriar
            try:
                DB_PATH.unlink()
            except Exception:
                pass
    ...
    if not csvs:
        # Mostrar o que foi encontrado para debug
        st.error(f"Nenhum CSV encontrado em '{CSV_DIR}'. "
                 f"Conteúdo da pasta: {list(CSV_DIR.glob('*')) if CSV_DIR.exists() else 'pasta não existe'}")
        return False

    st.info(f"Construindo banco a partir de {len(csvs)} arquivo(s)...")
